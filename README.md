...
Such workflow is not only time-consuming, but also tightly tied to the specific application features, which is too inflexiable to handle the variable developing requirements.

We propose a flexible and efficient undo framework which not only allows developers to complete undo/redo features in less time, but also ensures the **crash consistency** of the application.
