...
3. **编解码器按类型注册。** 核心节点与动作的编解码器由 substate 提供，`Property` 中 `QVariant` 的编解码器由 qsubstate 提供，用户节点类型由调用方注册。取代现在的 `ActionIOInterface`。`QVariant` 的编码分为两类：
   - `bool`、整数（`int`、`qint64` 等）、`double`、`QString` 与 `QByteArray` 由 qsubstate 以固定格式自行编码，不依赖 Qt 的序列化格式。整数统一以 64 位写出，读取时恢复原类型。
   - 其他类型，包括调用方的自定义类型，经由 `QDataStream` 编码。流版本固定为一个值，否则同一日志在不同 Qt 版本下的编码不同，AceTreeModel 同样将流版本固定为 `QDataStream::Qt_5_0`。自定义类型以类型名记录，调用方须在读写之前注册该类型及其流运算符。类型改名或删除后，记录了该类型的日志无法读取，这一责任属于调用方。
